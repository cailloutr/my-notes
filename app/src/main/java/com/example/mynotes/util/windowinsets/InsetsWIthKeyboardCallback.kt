...
class InsetsWithKeyboardCallback(
    ...
    override fun onApplyWindowInsets(v: View, insets: WindowInsetsCompat): WindowInsetsCompat {
        ...
        val types = when {
            // When the deferred flag is enabled, we only use the SystemBar() insets
            deferredInsets -> WindowInsetsCompat.Type.systemBars()
            // When the deferred flag is disabled, we use the combination of SystemBar() and ime() Insets
